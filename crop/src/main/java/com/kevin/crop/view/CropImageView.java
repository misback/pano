...
public class CropImageView extends TransformImageView {
    ...
    public Bitmap cropCircleImage() {
        ...
        int width = (int) (mCropRect.width() / currentScale);
        int height = (int) (mCropRect.height() / currentScale);
        Bitmap output = Bitmap.createBitmap(viewBitmap, left, top, width, height);
        Canvas canvas = new Canvas( output);

        final int color = 0xff424242;
        final Paint paint = new Paint();
        final Rect rect = new Rect( 0, 0, viewBitmap.getWidth(), viewBitmap.getHeight());
        ...
        //在画布上绘制一个圆
        canvas.drawCircle( viewBitmap.getWidth() / 2, viewBitmap.getHeight() / 2, viewBitmap.getWidth() / 2, paint);
        paint.setXfermode( new PorterDuffXfermode(PorterDuff.Mode.SRC_IN));
        canvas.drawBitmap( viewBitmap, rect, rect, paint);
        return output;
    }

...
void Utils::inject_mpeg4(std::string &strInFile, std::string &strOutFile, Metadata *pMetadata) {
    ...
    if (!inFile.is_open()) {
        ...
        LOGI("does not exist or do not have permission");
        return;
    }

    LOGI("inject mpeg4 start .....01");
    ...
    if (pMetadata->getAudio()) {
        ...
        if (!bRet) {
            ...
            LOGI("Error failed to insert spatial audio data");
        }
    }
    std::cout << "Saved file settings" << std::endl;
    LOGI("Saved file settings");
    ...
    if (!outFile.is_open()) {
        ...
        LOGI("could not create or do not have permission");
        return;
    }

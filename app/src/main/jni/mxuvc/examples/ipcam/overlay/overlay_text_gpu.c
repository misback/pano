...
//Max OSD length = 12
// static char *szText0="Z";
static char *szText0="quickfoxjumpedoveradog";
static char *szText1="#$!%%&*  *907654123";
...
int main(int argc, char **argv)
{
	...
	int font_size;
        ...
        if ( argc < 2 ){
                printf("USAGE: sudo overlay_text <path to font file> <size of font> [enable timer 0/1(default 0)] [enable frame numbering 0/1(default 0) <path to alpha channel(optional)>\n");
                return 0;
        }
	...
	for(channel=0; channel<OVCOUNT ; channel++)
    { 
    	overlay_font_params_t font;
    	// Please fill these params as per font configuration
    	font.size = 16;
    	font.start = 20;
    	font.end = 126;
    	font.width = 1;
    	font.imgwidth=16;
    	font.imgheight=2048;
    	ret = mxuvc_overlay_load_font_gpu(channel, &font, file, alpha);	
		if(ret < 0)
	  		return 1;
	...
	while(counter--) {
		...
		if(counter == 18)
		{
			...
			for(channel=0; channel < OVCOUNT; channel++)
			{
				...
				ovtext[ch][0].yoff = 0;
				ovtext[ch][0].idx = 0;
				ret = mxuvc_overlay_add_text_gpu(ch, &ovtext[ch][0], szText0, strlen(szText0));
					if(ret < 0)
						goto error;
#endif
				ovtext[ch][1].xoff = 0;
				...
				ovtext[ch][1].idx = 1;
				ret = mxuvc_overlay_add_text_gpu(ch, &ovtext[ch][1], szText1, strlen(szText1));
					if(ret < 0)
						goto error;
#if 1
				...
				ret = mxuvc_overlay_add_text_gpu(ch, &ovtext[ch][2], szDate, strlen(szDate));
					if(ret < 0)
						goto error;
				...
				ret = mxuvc_overlay_add_text_gpu(ch, &ovtext[ch][3], szText2, strlen(szText2));
					if(ret < 0)
						goto error;
				...
				ret = mxuvc_overlay_add_text_gpu(ch, &ovtext[ch][4], szText2, strlen(szText2));
					if(ret < 0)
						goto error;
				#endif
		...
		if(counter == 15 && timer == 1 )
		{
			...
			for(channel=0; channel < OVCOUNT; channel++)
			{
				...
				ret = mxuvc_overlay_set_time_gpu(ch, &ovtime, &btime);
				if(ret < 0)
					goto error;
				ret = mxuvc_overlay_show_time_gpu(ch);
				if(ret < 0)
					goto error;
			}
		}
		...
		if(counter == 2)
		{
			...
			for(channel=0; channel<OVCOUNT ; channel++)
			{
				...
				int ch = channel;		
				printf("from ch: %d\n",ch);
				ret = mxuvc_overlay_remove_text_gpu(channel, ovtext[ch][0].idx);
				if(ret < 0)
					goto error;

				ret = mxuvc_overlay_remove_text_gpu(channel, ovtext[ch][1].idx);
				if(ret < 0)
					goto error;

				ret = mxuvc_overlay_remove_text_gpu(channel, ovtext[ch][2].idx);
				if(ret < 0)
					goto error;

				ret = mxuvc_overlay_remove_text_gpu(channel, ovtext[ch][3].idx);
				if(ret < 0)
					goto error;

				ret = mxuvc_overlay_remove_text_gpu(channel, ovtext[ch][4].idx);
				if(ret < 0)
					goto error;

				ret = mxuvc_overlay_hide_time_gpu(channel);
				if(ret < 0)
					goto error;
			}
		}

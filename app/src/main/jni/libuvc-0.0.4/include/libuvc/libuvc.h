...
uvc_error_t uvc_init2(uvc_context_t **ctx,
		struct libusb_context *usb_ctx, const char *usbfs);
...
uvc_error_t uvc_get_stream_ctrl_format_size_fps(
		...
		int width, int height,
		int min_fps, int max_fps);	// XXX added

uvc_error_t uvc_probe_stream_ctrl(
		uvc_device_handle_t *devh,
		uvc_stream_ctrl_t *ctrl);

uvc_error_t uvc_get_frame_desc(
		uvc_device_handle_t *devh,
		uvc_stream_ctrl_t *ctrl,
...
uvc_error_t uvc_mjpeg2yuv444(uvc_frame_t *in, uvc_frame_t *out);	// XXX
#endif

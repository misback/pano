...
#include <libgen.h>
#ifdef __ANDROID__
	#define UVC_DEBUG(format, ...) __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, "[%s:%d] %s : " format "\n", \
							basename(__FILE__), __LINE__, __FUNCTION__, ##__VA_ARGS__)
	#define UVC_ERROR(format, ...) __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "[%s:%d] %s : " format "\n", \
							basename(__FILE__), __LINE__, __FUNCTION__, ##__VA_ARGS__)
	#define UVC_ENTER() __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, "[%s:%d] begin %s", \
							basename(__FILE__), __LINE__, __FUNCTION__)
	#define UVC_EXIT(code) __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, "[%s:%d] end %s (%d)", \
							basename(__FILE__), __LINE__, __FUNCTION__, code)
	#define UVC_EXIT_VOID() __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, "[%s:%d] end %s", \
							basename(__FILE__), __LINE__, __FUNCTION__)
#else
...
#endif
#else
	#define UVC_DEBUG(...) do {} while(0)
#ifdef __ANDROID__
	#define UVC_ERROR(...) __android_log_print(ANDROID_LOG_ERROR, LOG_TAG,__VA_ARGS__)
#else
	#define UVC_ERROR(...) do {} while(0)
#endif
	#define UVC_ENTER() do {} while(0)
	#define UVC_EXIT(code) do {} while(0)
	#define UVC_EXIT_VOID() do {} while(0)
#endif

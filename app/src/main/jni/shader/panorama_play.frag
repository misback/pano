const static char* panorama_play_frag    =   STRINGIFY(
    ...
    void main(void){
        ...
        if(u_FaceSize>0.0){
            ...
            if(v_TextureUV.x>=startX && v_TextureUV.x<=endX && v_TextureUV.y>startY && v_TextureUV.y<endY){
                if(v_TextureUV.x>=(1.0-startSubX)){
                     vec4 r_ImgColor = texture2D(u_LutTexture, vec2((v_TextureUV.x-1.0+startSubX)/u_FaceSize, (v_TextureUV.y-startY)/doubleSize));
                     r_Color = vec4(r_Color.rgb*(1.0 - r_ImgColor.a)+r_ImgColor.rgb*r_ImgColor.a, 1.0);
                }else if(endSubX>=v_TextureUV.x){
                     vec4 r_ImgColor = texture2D(u_LutTexture, vec2((endSubX-v_TextureUV.x)/u_FaceSize, (v_TextureUV.y-startY)/doubleSize));
                     r_Color = vec4(r_Color.rgb*(1.0 - r_ImgColor.a)+r_ImgColor.rgb*r_ImgColor.a, 1.0);
                }else{
                     vec4 r_ImgColor = texture2D(u_LutTexture, vec2((v_TextureUV.x-startX)/u_FaceSize, (v_TextureUV.y-startY)/doubleSize));
                     r_Color = vec4(r_Color.rgb*(1.0 - r_ImgColor.a)+r_ImgColor.rgb*r_ImgColor.a, 1.0);
                }
            }
        }
        ...
        if(u_GpuImage == 1){
            ...
            vec4 centerDown = texture2D(u_CameraTextureOES, vec2(v_TextureUV.x, maxY));
            vec4 leftCenter = texture2D(u_CameraTextureOES, vec2(minX, v_TextureUV.y));
            vec4 rightCenter = texture2D(u_CameraTextureOES, vec2(maxX, v_TextureUV.y));
            vec4 centerUp = texture2D(u_CameraTextureOES, vec2(v_TextureUV.x, minY));

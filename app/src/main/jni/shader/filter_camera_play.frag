const char* filter_camera_play_frag    =   STRINGIFY(
        ...
        if(u_FaceSize>0.0){
            ...
            if(uv.x>=startX && uv.x<=endX && uv.y>startY && uv.y<endY){
                if(uv.x>=(1.0-startSubX)){
                     vec4 r_ImgColor = texture2D(u_LutTexture, vec2((uv.x-1.0+startSubX)/u_FaceSize, (uv.y-startY)/doubleSize));
                     r_Color = vec4(r_Color.rgb*(1.0 - r_ImgColor.a)+r_ImgColor.rgb*r_ImgColor.a, 1.0);
                }else if(endSubX>=uv.x){
                     vec4 r_ImgColor = texture2D(u_LutTexture, vec2((endSubX-uv.x)/u_FaceSize, (uv.y-startY)/doubleSize));
                     r_Color = vec4(r_Color.rgb*(1.0 - r_ImgColor.a)+r_ImgColor.rgb*r_ImgColor.a, 1.0);
                }else{
                     vec4 r_ImgColor = texture2D(u_LutTexture, vec2((uv.x-startX)/u_FaceSize, (uv.y-startY)/doubleSize));
                     r_Color = vec4(r_Color.rgb*(1.0 - r_ImgColor.a)+r_ImgColor.rgb*r_ImgColor.a, 1.0);
                }
            }
        }
